...
const level = () => {
  ...
  const isDevelopment = envNode === "development";
...
const format = winston.format.combine(
  ...
  winston.format.printf((info) => {
    ...
    // If message is an object, stringify the whole thing
    const logMessage =
      typeof message === "object" ? JSON.stringify(message, null, 2) : message;

    // If there are additional fields, include them
    const additionalFields =
      Object.keys(rest).length > 0 ? `\n${JSON.stringify(rest, null, 2)}` : "";

    return `[${timestamp}] [${level}] : ${logMessage}${additionalFields}`;
  })
  // winston.format.json()
);
